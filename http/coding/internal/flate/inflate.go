...
func (f *decompressor) huffmanReader(data []byte) (extra []byte, cont bool) {
	const (
		eReadLiteral = iota // Zero value must be stateInit
		eCopyHistory
	)
	...
	// Read literal and/or (length, distance) according to RFC section 3.2.3.
	{
		...
		for n := uint(f.hl.maxRead); ; {
			for ; fnb < n; fnb += 8 {
				...
				f.roffset++
				fb |= uint32(data[0]) << (fnb & regSizeMaskUint32)
			}
			...
			if n <= fnb {
				if n == 0 {
					f.b, f.nb = fb, fnb
					// TODO: remove debugDecode branches
					if debugDecode {
						fmt.Println("huffsym: n==0")
					}
					f.err = CorruptInputError(f.roffset)
